...
static const Gfx uname_setup_dl[] = {
    ...
    gsDPSetOtherMode(G_AD_DISABLE | G_CD_DISABLE | G_CK_NONE | G_TC_FILT
        ...
        AA_EN | Z_CMP | Z_UPD | CVG_DST_CLAMP | ZMODE_OPA | FORCE_BL |
        GBL_c1(G_BL_CLR_IN, G_BL_A_IN, G_BL_CLR_MEM, G_BL_1MA) |
        GBL_c2(G_BL_CLR_IN, G_BL_A_IN, G_BL_CLR_MEM, G_BL_1MA)),
}

#define RAM_END 0x80800000
#define UNAME_TEX_BEGIN (RAM_END - MAX_TWITCH_MESSAGES * UNAME_TEX_SIZE * sizeof(u64))
#define VTX_BEGIN (UNAME_TEX_BEGIN - MAX_TWITCH_MESSAGES * 4 * sizeof(Vtx))

static inline u64 *GetMessageTexture(u16 m) {
    return (u64*)(UNAME_TEX_BEGIN + m * UNAME_TEX_SIZE * sizeof(u64));
}

static inline Vtx *GetMessageVerts(u16 m) {
    return (Vtx*)(VTX_BEGIN + m * 4 * sizeof(Vtx));
}
...
static void SetUpMessage(u16 m, TwitchMessage *msg) {
    ...
    SetupRectangle(verts, basex, basey, basez, 128, 16);
    ...
    s32 texcol = 0;
    while(byte < 19){
        u32 temp = uname_encoded[byte++];
        temp |= (u32)uname_encoded[byte++] << 8; //will overread on the last
        temp |= (u32)uname_encoded[byte++] << 16; //but that's okay
    ...
    if(texcol < UNAME_TEX_SIZE){
        bzero(&tex[texcol], &tex[UNAME_TEX_SIZE], 
            (UNAME_TEX_SIZE-texcol) * sizeof(u64));
    }
}
...
static void init(Entity *en, GlobalContext *globalCtx) {
    bzero(VTX_BEGIN, UNAME_TEX_BEGIN - VTX_BEGIN);
}
...
static void update(Entity *en, GlobalContext *globalCtx){
    for(s32 m=0; m<MAX_TWITCH_MESSAGES; ++m){
        ...
        if(msg->timer == 0xFF){
            msg->timer = 1;
            SetUpMessage(msg, GetMessageTexture(m));
        }else if(msg->timer != 0){
            ++msg->timer;
...
static void draw(Entity *en, GlobalContext *globalCtx) {
    Matrix_Translate(msg->pos.x, msg->pos.y, msg->pos.z, MTXMODE_NEW);
    Matrix_RotateRPY(0, 0, en->actor.shape.rot.y, MTXMODE_APPLY);
    ...
    for(s32 m=0; m<MAX_TWITCH_MESSAGES; ++m){
        ...
        gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, msg->r, msg->g, msg->b, msg->a);
        gDPLoadMultiBlock_4b(POLY_OPA_DISP++, GetMessageTexture(m), 
            0, G_TX_RENDERTILE, G_IM_FMT_I, 16, UNAME_TEX_SIZE, 0,
            G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, 
            G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
        gSPVertex(POLY_OPA_DISP++, GetMessageVerts(m), 4, 0);
